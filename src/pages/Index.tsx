...
const Index = () => {
  ...
  useEffect(() => {
    const fetchInterviewerData = async () => {
      ...
      try {
        ...
        } = await supabase.from('interviewers').select('id').eq('code', interviewerCode).limit(1);
        if (interviewerError) {
        ...
        const {
          data: sessions,
          error: sessionsError
        } = await supabase.from('sessions').select('id, start_time, end_time').eq('interviewer_id', interviewerId).not('end_time', 'is', null);
        if (sessionsError) {
        ...
        if (sessions && sessions.length > 0) {
          ...
          if (sessionIds.length > 0) {
            ...
            const {
              count,
              error: countError
            } = await (supabase as any).from('interviews').select('id', {
              count: 'exact'
            }).in('session_id', sessionIds).not('result', 'is', null);
            if (countError) {
  ...
  return <MainLayout>
      <div className="flex flex-col items-center justify-center min-h-[80vh] max-w-md mx-auto">
        ...
        {unsyncedCount > 0 && <Alert className="mb-4 w-full bg-blue-50 border-blue-200 text-blue-800">
            <AlertDescription className="flex items-center justify-between">
              <span>
                {unsyncedCount} offline {unsyncedCount === 1 ? 'session' : 'sessions'} not yet synchronized
              </span>
              {isOnline() && <button onClick={performSync} className="px-2 py-1 bg-blue-100 hover:bg-blue-200 rounded text-xs">
                  Sync Now
                </button>}
            </AlertDescription>
          </Alert>}
        ...
        <SessionForm interviewerCode={interviewerCode} setInterviewerCode={setInterviewerCode} isRunning={isRunning} setIsRunning={setIsRunning} startTime={startTime} setStartTime={setStartTime} startLocation={startLocation} setStartLocation={setStartLocation} activeSession={activeSession} setActiveSession={setActiveSession} isPrimaryUser={isPrimaryUser} switchUser={switchUser} endSession={endSession} startSession={startSession} offlineSessionId={offlineSessionId} validateInterviewerCode={validateInterviewerCode} />
        
        {interviewerCode && <div className="w-full space-y-6 mt-6">
