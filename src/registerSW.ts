...
export const requestSync = async (): Promise<string | undefined> => {
...
// Example of a more complete service worker registration
if ('serviceWorker' in navigator) {
        ...
        installingWorker.addEventListener('statechange', () => {
          if (installingWorker.state === 'installed') {
            ...
            if (navigator.serviceWorker.controller) {
              ...
              // but the previous service worker will still be in control unless
  ...
  navigator.serviceWorker.addEventListener('message', (event) => {
    if (event.data && event.data.type === 'RELOAD_REQUIRED') {
      console.log('Service worker says: Reload is required');
      logSync('ServiceWorker', 'ReloadRequired', 'info', 'Service worker says: Reload is required');
      // Optionally, display a notification to the user or automatically reload the page
  ...
  navigator.serviceWorker.addEventListener('waiting', (event) => {
    ...
    logSync('ServiceWorker', 'WaitingToActivate', 'info', 'Service worker waiting to activate.');
  });
  ...
  navigator.serviceWorker.addEventListener('redundant', (event) => {
    ...
    logSync('ServiceWorker', 'Redundant', 'info', 'Service worker became redundant.');
  });

  // Detect when a service worker is activated
  navigator.serviceWorker.addEventListener('activate', (event) => {
    console.log('Service worker activated.');
    logSync('ServiceWorker', 'Activated', 'info', 'Service worker activated.');
  });

  // Detect when a service worker is installed
  navigator.serviceWorker.addEventListener('install', (event) => {
    console.log('Service worker installed.');
    logSync('ServiceWorker', 'Installed', 'info', 'Service worker installed.');
  });

  // Detect when a service worker is fetched
  navigator.serviceWorker.addEventListener('fetch', (event) => {
    //console.log('Service worker fetched:', event.request.url);
    //logSync('ServiceWorker', 'Fetched', 'info', `Service worker fetched: ${event.request.url}`);
  });

  // Detect when a service worker is controlled
  navigator.serviceWorker.addEventListener('controllerchange', (event) => {
    console.log('Service worker controlled.');
    logSync('ServiceWorker', 'Controlled', 'info', 'Service worker controlled.');
  });
}
